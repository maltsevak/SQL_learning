...
SELECT concat('Кампания № ', ads_campaign) as ads_campaign ,
       round((sum(user_avg_check)::float / count(distinct user_id))::numeric,
             2) as avg_check
FROM   (SELECT user_id,
               ads_campaign,
               sum(price)::float / count(distinct order_id) as user_avg_check
        FROM   (SELECT user_id,
                       order_id,
                       case when user_id in (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732,
                            ...
                            when user_id in (8629, 8630, 8644, 8646, 8650, 8655, 8659, 8660, 8663, 8665, 8670,
                                             ...
                                             10113, 10131) then '2' end as ads_campaign
                FROM   user_actions
                WHERE  time::date >= '2022-09-01'
                   ...
                   and user_id in (SELECT user_id
                                FROM   user_actions
                                GROUP BY user_id having min(time)::date >= '2022-09-01'))q1
            LEFT JOIN (SELECT order_id,
                              unnest(product_ids) as product_id
                       FROM   orders
                       WHERE  creation_time::date >= '2022-09-01')q2 using(order_id)
            LEFT JOIN products using(product_id)
        WHERE  ads_campaign is not null
        GROUP BY user_id, ads_campaign)q3
--WHERE ads_campaign IS NOT NULL
